...
void MonixKernel::Initialize(Security::AuthManager* auth) {
  auth_ = auth;
  phase_ = KernelPhase::Startup;
  phaseStartMs_ = GetTickCount64();
  bootTimeMs_ = GetTickCount64();
  lastTestMs_ = 0;
  events_.Clear();
  results_.clear();
  currentGroup_ = 0;
  ...
  authAttempts_ = 0;
  authError_.clear();
  statusMessage_ = L"Starting kernel diagnostics...";
}
...
void MonixKernel::TriggerCrash(const char* subsystem, const char* error, const char* description) {
  ...
  crashSubsystem_ = subsystem;
  crashTest_ = error;
  crashDetail_ = description;
  statusMessage_ = L"FATAL SYSTEM ERROR";
}

void MonixKernel::SkipTests() {
  if (phase_ != KernelPhase::Diagnostics && phase_ != KernelPhase::Startup) return;
  testsSkipped_ = true;
  const auto& groups = diag_.Groups();
  std::size_t totalTests = 0;
  for (const auto& g : groups) totalTests += g.tests.size();
  skipCount_ += static_cast<std::uint32_t>(totalTests - results_.size());
  events_.Emit(monix::kernel::EventSubsystem::Kernel, monix::kernel::EventSeverity::Warning, "TESTS_SKIPPED", "Developer skip: Ctrl+Shift+K");
  TransitionToReady();
  statusMessage_ = L"TESTS SKIPPED (DEV MODE)";
}
