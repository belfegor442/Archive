...
CompileEntryResult ShaderLibraryCompiler::compileSlangOrGlsl(ShaderLibraryEntry& entry) {
    ...
    entry.compileDurationMs = std::chrono::duration<double, std::milli>(t1 - t0).count();
    ...
    // Propagate cache status from ShaderRuntime (15.2)
    entry.lastCacheStatus = runtime_->lastCacheStatus();

    // Propagate SPIR-V size (15)
    entry.spirvSizeBytes = 0;
    const auto& mod = compileResult.value();
    for (const auto& stage : mod.stages) {
        entry.spirvSizeBytes += stage.spirv.size();
    }
    ...
    TransactionalShaderState& txState)
{
    ...
    if (!compileResult.success) {
        ...
        e->totalActivationMs = std::chrono::duration<double, std::milli>(tTotal1 - tTotal0).count();
        return result;
    }
    ...
    if (renderer_) {
        if (e2->extension == ".slangp" || e2->extension == ".glslp") {
            auto tVal0 = std::chrono::steady_clock::now();
            auto loadResult = renderer_->loadPreset(e2->path);
            auto tVal1 = std::chrono::steady_clock::now();
            e2->validationDurationMs = std::chrono::duration<double, std::milli>(tVal1 - tVal0).count();
            ...
            if (!loadResult) {
                ...
                auto tTotal1 = std::chrono::steady_clock::now();
                e2->totalActivationMs = std::chrono::duration<double, std::milli>(tTotal1 - tTotal0).count();
                return result;
            }
        } else {
            auto tVal0 = std::chrono::steady_clock::now();
    ...
    TransactionalShaderState* txState)
{
    ...
    if (txState) {
        ...
        if ((e->extension == ".slangp" || e->extension == ".glslp") && renderer_) {
