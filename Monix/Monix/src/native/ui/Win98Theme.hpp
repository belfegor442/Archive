...
public:
  ...
  static void DrawUpdateDialog(HDC dc, const Canvas& c, const monix::UpdateState& us) {
    ...
    const int dlgW = 420;
    const int dlgH = 280;
    int dlgX = c.rect.left + static_cast<int>((c.rect.right - c.rect.left - dlgW * c.scale) / 2);
    int dlgY = c.rect.top + static_cast<int>((c.rect.bottom - c.rect.top - dlgH * c.scale) / 2);
    RECT dlg = { dlgX, dlgY, dlgX + static_cast<int>(dlgW * c.scale), dlgY + static_cast<int>(dlgH * c.scale) };

    Fill(dc, dlg, kWin98Gray);
    Win98Bevel(dc, dlg, false);

    RECT titleBar = { dlg.left, dlg.top, dlg.right, dlg.top + static_cast<int>(22 * c.scale) };
    ...
    RECT closeBtn = { dlg.right - static_cast<int>(20 * c.scale), dlg.top + static_cast<int>(2 * c.scale),
                      dlg.right - static_cast<int>(4 * c.scale), dlg.top + static_cast<int>(20 * c.scale) };
    ...
    int cy = dlg.top + static_cast<int>(34 * c.scale);
    int innerX = dlg.left + static_cast<int>(16 * c.scale);
    int innerW = dlgW - 32;

    if (us.checking) {
      Text(dc, R(c, 0, cy, dlgW, 20), L"Checking for updates...", c.fonts->smallFont, kWin98Black, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
      cy += static_cast<int>(30 * c.scale);
    } else if (us.updateAvailable) {
      Text(dc, R(c, 0, cy, dlgW, 20), L"A new version is available!", c.fonts->menuFont, kWin98Navy, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
      cy += static_cast<int>(28 * c.scale);

      Text(dc, R(c, 0, cy, dlgW, 20), L"You are running: " + us.currentVersion, c.fonts->smallFont, kWin98DarkGray, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
      cy += static_cast<int>(18 * c.scale);
      Text(dc, R(c, 0, cy, dlgW, 20), L"Latest version:  " + us.latestVersion, c.fonts->smallFont, kWin98BrightGreen, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
      cy += static_cast<int>(24 * c.scale);

      RECT notesBg = { innerX, cy, innerX + static_cast<int>(innerW * c.scale), cy + static_cast<int>(80 * c.scale) };
      Fill(dc, notesBg, kWin98White);
      Win98Bevel(dc, notesBg, true);
      if (!us.releaseNotes.empty()) {
        RECT notesText = { notesBg.left + static_cast<int>(6 * c.scale), notesBg.top + static_cast<int>(4 * c.scale),
                           notesBg.right - static_cast<int>(6 * c.scale), notesBg.bottom - static_cast<int>(4 * c.scale) };
      ...
      Text(dc, R(c, 0, cy, dlgW, 20), L"You are running the latest version!", c.fonts->menuFont, kWin98BrightGreen, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
      cy += static_cast<int>(24 * c.scale);
      Text(dc, R(c, 0, cy, dlgW, 20), L"Version: " + us.currentVersion, c.fonts->smallFont, kWin98DarkGray, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    }

    const int btnW = 90;
    const int btnH = 24;
    int btnY = dlg.bottom - static_cast<int>(38 * c.scale);

    if (us.updateAvailable) {
      int okX = dlg.left + static_cast<int>((dlgW / 2 - btnW - 10) * c.scale);
      RECT okBtn = { okX, btnY, okX + static_cast<int>(btnW * c.scale), btnY + static_cast<int>(btnH * c.scale) };
      ...
      int cxBtn = okX + static_cast<int>((btnW + 20) * c.scale);
      RECT cancelBtn = { cxBtn, btnY, cxBtn + static_cast<int>(btnW * c.scale), btnY + static_cast<int>(btnH * c.scale) };
      ...
      int okX = dlg.left + static_cast<int>((dlgW / 2 - btnW / 2) * c.scale);
      RECT okBtn = { okX, btnY, okX + static_cast<int>(btnW * c.scale), btnY + static_cast<int>(btnH * c.scale) };
      Fill(dc, okBtn, us.hoverButton == 0 ? kWin98Highlight : kWin98ButtonFace);
      Win98Bevel(dc, okBtn, false);
