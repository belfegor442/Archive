...
// GL headers retained for type definitions (GLuint, GLenum, etc.) during migration
#include <GL/GL.h>
#include <GL/glext.h>

// Custom GL typedefs not in Windows SDK headers
...
#include "transitions/ScanlineBlender.hpp"
...
const std::array<SettingId, 4>& GeneralExtendedSettings() {
  static const std::array<SettingId, 4> settings {
    SettingId::Language,
    SettingId::FontFaceIndex,
    SettingId::ShowFps,
...
private:
  ...
  std::unique_ptr<monix::TransitionManager> transitionManager_;
  std::vector<uint8_t> transitionOldFrame_;
  std::vector<uint8_t> transitionBlendBuffer_;
  int transitionFrameWidth_ = 0;
  int transitionFrameHeight_ = 0;
  LARGE_INTEGER lastTransitionTick_ {};
...
MonixApp::MonixApp() : paths_(ResolveAppPaths()) {
  ...
  transitionManager_ = std::make_unique<monix::TransitionManager>();
  monix::TransitionConfig tc;
  tc.enabled = config_.transitionEnabled;
  tc.durationMs = config_.transitionDurationMs;
  tc.scanlineWidth = config_.transitionScanlineWidth;
  tc.intensity = config_.transitionIntensity;
  tc.distortion = config_.transitionDistortion;
  tc.noiseAmount = config_.transitionNoise;
  tc.flickerAmount = config_.transitionFlicker;
  transitionManager_->setConfig(tc);

  transitionManager_->setEventCallback([this](const monix::TransitionEvent& e) {
    using Type = monix::TransitionEvent::Type;
    if (e.type == Type::Started) {
      monix::Tab tabs[] = { Tab::Log, Tab::Tasks, Tab::Hardware, Tab::Network, Tab::Scram, Tab::Settings };
      int fromIdx = static_cast<int>(e.fromTab);
      int toIdx = static_cast<int>(e.toTab);
      const wchar_t* tabNames[] = { L"LOG", L"TASKS", L"HARDWARE", L"NETWORK", L"SCRAM", L"SETTINGS" };
      wchar_t msg[256];
      swprintf_s(msg, L"TRANSITION START FROM=%s TO=%s TYPE=%S DURATION=%.0f",
        (fromIdx >= 0 && fromIdx < 6) ? tabNames[fromIdx] : L"?",
        (toIdx >= 0 && toIdx < 6) ? tabNames[toIdx] : L"?",
        monix::TransitionTypeName(e.transitionType),
        e.durationMs);
      PushLog(L"SYSTEM", L"INFO", msg, ColorRole::Primary, L"Transition", L"TransitionManager");
    } else if (e.type == Type::Completed) {
      PushLog(L"SYSTEM", L"INFO", L"TRANSITION COMPLETE", ColorRole::Primary, L"Transition", L"TransitionManager");
    }
  });
...
void MonixApp::WriteDefaultConfigIfMissing() const {
  ...
  output << "crt_enabled=true\n";
  output << "crt_scanline_spacing=3\n";
  output << "crt_ghost_offset=1\n";
  output << "crt_curvature_strength=0.02\n";
  output << "crt_scanline_intensity=0.30\n";
  output << "crt_chromatic_aberration=0.03\n";
  output << "crt_phosphor_glow=0.12\n";
  output << "crt_flicker_amount=0.012\n";
  output << "crt_noise_amount=0.018\n";
  output << "crt_vignette_strength=0.22\n";
  output << "crt_sharpness=0.55\n";
...
SettingMutation MonixApp::AdjustSetting(SettingId id, int direction) {
  ...
  switch (id) {
    ...
    case SettingId::ThemeMode: {
      int index = std::clamp(config_.themeMode, 0, 2);
      index += step;
      if (index < 0) index = 2;
      if (index > 2) index = 0;
...
void MonixApp::CreateUiFonts() {
  ...
  tabFont_ = CreateFontW(-scale(30), 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, fontCharset, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, fontFace_.c_str());
  bodyFont_ = CreateFontW(-scale(22), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, fontCharset, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, fontFace_.c_str());
  smallFont_ = CreateFontW(-scale(17), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, fontCharset, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, fontFace_.c_str());
  titleFont_ = CreateFontW(-scale(34), 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, fontCharset, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, fontFace_.c_str());
  logFont_ = CreateFontW(-scale(18), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, fontCharset, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FIXED_PITCH | FF_MODERN, fontFace_.c_str());
...
static LONG CALLBACK CrashVehHandler(EXCEPTION_POINTERS* ep) {
  if (ep && ep->ExceptionRecord && ep->ExceptionRecord->ExceptionCode == 0xC0000005) {
    HANDLE h = CreateFileA("C:\\Users\\orimu\\AppData\\Local\\Temp\\monix_crash.txt",
  ...
  if (!paths_.crtShaderFile.empty() && std::filesystem::exists(paths_.crtShaderFile)) {
    ...
    if (loadStatus) {
      ...
      if (compiled) {
        ...
        // Backend was already loaded by loadPreset when outputExtent was set
...
bool MonixApp::RenderOpenGlFrame(const RECT& clientRect) {
  ...
  // Render GDI UI to CPU bitmap
  if (!EnsureOpenGlUiSurface(width, height)) {
    return false;
  }
  Render(openGl_.uiDc, clientRect);
  GdiFlush();

  if (transitionManager_ && transitionManager_->isTransitioning()) {
    LARGE_INTEGER now;
    QueryPerformanceCounter(&now);
    double elapsedMs = 16.0;
    if (lastTransitionTick_.QuadPart != 0) {
      elapsedMs = static_cast<double>(now.QuadPart - lastTransitionTick_.QuadPart) /
                  static_cast<double>(qpcFrequency_.QuadPart) * 1000.0;
    }
    lastTransitionTick_ = now;
    transitionManager_->update(elapsedMs);

    if (!transitionOldFrame_.empty() && openGl_.uiPixels &&
        transitionFrameWidth_ == openGl_.uiWidth && transitionFrameHeight_ == openGl_.uiHeight) {
      size_t frameSize = openGl_.uiWidth * openGl_.uiHeight * 4;
      if (transitionBlendBuffer_.size() < frameSize) {
        transitionBlendBuffer_.resize(frameSize);
      }
      monix::TransitionState ts = transitionManager_->gpuState();
      monix::ScanlineBlender::blend(
        transitionBlendBuffer_.data(),
        transitionOldFrame_.data(),
        static_cast<const uint8_t*>(openGl_.uiPixels),
        static_cast<uint32_t>(openGl_.uiWidth),
        static_cast<uint32_t>(openGl_.uiHeight),
        static_cast<uint32_t>(openGl_.uiWidth * 4),
        ts);

      monix::renderer_vk::FrameImage srcFrame{};
      srcFrame.pixels = transitionBlendBuffer_.data();
      srcFrame.extent = {static_cast<uint32_t>(openGl_.uiWidth), static_cast<uint32_t>(openGl_.uiHeight)};
      srcFrame.bytes = frameSize;
      monix::renderer_vk::FrameContext fc{};
      openGl_.shaderRenderer->render(fc, &srcFrame);
      ++openGl_.frameCount;
    ...
    QueryPerformanceCounter(&now);
    lastTransitionTick_ = {};
    transitionOldFrame_.clear();
    transitionBlendBuffer_.clear();
  }

  if (openGl_.presetLoaded && openGl_.shaderRenderer && openGl_.vk.isInitialized()) {
    // Preset loaded: ShaderRenderer::render() manages beginFrame/render/present internally
    monix::renderer_vk::FrameImage srcFrame{};
...
void MonixApp::UpdateScramSummary(const Snapshot& snapshot) {
  ...
  if (state_.hasPreviousSnapshot) {
    ...
    if (snapshot.regHashAudit != prevRegHashAudit_ && prevRegHashAudit_ != 0) {
      ...
      diagnostics.push_back(L"Security policy hash changed.");
      risk += 16;
    }
...
bool MonixApp::HandleSettingsClick(const RECT& clientRect, POINT point) {
  ...
  if (state_.shaderBrowser.subTab == SettingsSubTab::Shaders) {
    ...
    if (hitFilterTab(ShaderLanguageFilter::SlangPreset)) return true;
    ...
    auto hitActionBtn = [&](const wchar_t* label) -> bool {
      ...
      if (PtInRect(&btn, point)) {
        if (wcscmp(label, L"IMPORT") == 0) {
          ...
          ofn.hwndOwner = hwnd_;
          ofn.lpstrFilter = "GLSL\0*.glsl\0Slang\0*.slang\0SlangPreset\0*.slangp\0All\0*.*\0";
  ...
  switch (message) {
    ...
    case WM_LBUTTONDOWN: {
      ...
      if (!state_.intro.active) {
        ...
        if (HitTestTabs(client, point, tab)) {
          if (tab != state_.activeTab && transitionManager_ && transitionManager_->enabled()) {
            Tab oldTab = state_.activeTab;
            if (openGl_.uiPixels && openGl_.uiWidth > 0 && openGl_.uiHeight > 0) {
              size_t frameSize = openGl_.uiWidth * openGl_.uiHeight * 4;
              transitionOldFrame_.assign(
                static_cast<const uint8_t*>(openGl_.uiPixels),
                static_cast<const uint8_t*>(openGl_.uiPixels) + frameSize);
              transitionFrameWidth_ = openGl_.uiWidth;
              transitionFrameHeight_ = openGl_.uiHeight;
            }
            state_.activeTab = tab;
            transitionManager_->requestTransition(oldTab, tab, monix::TransitionType::Scanline);
          } else {
            state_.activeTab = tab;
          }
          InvalidateRect(hwnd, nullptr, FALSE);
    ...
    case WM_MOUSEMOVE: {
      ...
      if (state_.taskMenu.visible) {
        ...
        if (PtInRect(&state_.taskMenu.rect, point)) {
          state_.taskMenu.hoverIndex = (point.y - state_.taskMenu.rect.top) / kTaskMenuItemHeight;
        } else {
    ...
    case WM_KEYDOWN: {
      ...
      if (wParam >= '1' && wParam <= '6') {
        ...
        if (newTab != state_.activeTab && transitionManager_ && transitionManager_->enabled()) {
          Tab oldTab = state_.activeTab;
          if (openGl_.uiPixels && openGl_.uiWidth > 0 && openGl_.uiHeight > 0) {
            size_t frameSize = openGl_.uiWidth * openGl_.uiHeight * 4;
            transitionOldFrame_.assign(
          ...
          transitionManager_->requestTransition(oldTab, newTab, monix::TransitionType::Scanline);
        } else {
          state_.activeTab = newTab;
        }
        InvalidateRect(hwnd, nullptr, FALSE);
...
#if 0 // Dead GL code removed - pipeline no longer used
      // Debug pass index navigation (when debug view is active)
      if (openGl_.debugViewMode == 1 && openGl_.pipeline.loaded) {
        int maxPass = static_cast<int>(openGl_.pipeline.compiledPasses.size()) - 1;
        if (wParam == VK_UP) {
          openGl_.debugPassIndex = std::max(0, openGl_.debugPassIndex - 1);
          SetToast(L"Debug: Pass " + std::to_wstring(openGl_.debugPassIndex));
          InvalidateRect(hwnd, nullptr, FALSE);
          return 0;
        }
        if (wParam == VK_DOWN) {
          openGl_.debugPassIndex = std::min(maxPass, openGl_.debugPassIndex + 1);
          SetToast(L"Debug: Pass " + std::to_wstring(openGl_.debugPassIndex));
          InvalidateRect(hwnd, nullptr, FALSE);
          return 0;
        }
...
#if 0 // Dead GL code removed - pipeline no longer used
      // Shift+F4: cycle presets (replaces old F4 preset cycling)
      if (wParam == VK_F4 && (GetKeyState(VK_SHIFT) & 0x8000)) {
        if (!openGl_.discoveredPresets.empty()) {
          if (openGl_.functionsLoaded && openGl_.dc && openGl_.rc) {
            wglMakeCurrent(openGl_.dc, openGl_.rc);
            openGl_.pipeline.destroy(openGl_.gl);
          }
          openGl_.currentPresetIndex = (openGl_.currentPresetIndex + 1) % static_cast<int>(openGl_.discoveredPresets.size());
          auto& preset = openGl_.discoveredPresets[openGl_.currentPresetIndex];
          if (openGl_.functionsLoaded && openGl_.dc && openGl_.rc) {
            wglMakeCurrent(openGl_.dc, openGl_.rc);
            // Phase 11: Set capture/compare modes before loading
            if (parityCapture_ || parityCompare_) {
              openGl_.pipeline.visualParity.captureEnabled = parityCapture_;
              openGl_.pipeline.visualParity.compareEnabled = parityCompare_;
              if (!parityRefDir_.empty()) openGl_.pipeline.visualParity.referenceDir = parityRefDir_;
            }
            // Phase 12: Set runtime state validation mode before loading
            if (runtimeStateEnabled_) {
              openGl_.pipeline.runtimeState.enabled = true;
              if (!retroarchSnapshotPath_.empty()) {
                openGl_.pipeline.runtimeState.loadRetroArchSnapshot(std::string(retroarchSnapshotPath_.begin(), retroarchSnapshotPath_.end()));
              }
            }
            if (openGl_.pipeline.load(preset, openGl_.gl, openGl_.status, paths_.rootDir)) {
              std::wstring name = Utf8ToWide(preset.name);
              PushLog(L"OPENGL", L"INFO", L"Pipeline: " + name + L" (" + std::to_wstring(preset.passes.size()) + L" passes)", ColorRole::UserInput, L"opengl", L"shader", L"name=" + name);
              SetToast(L"Shader: " + name);
            } else {
              SetToast(L"Shader error: " + Utf8ToWide(openGl_.pipeline.error.substr(0, 80)));
            }
          }
        }
        InvalidateRect(hwnd, nullptr, FALSE);
...
#if 0 // Dead GL code removed - pipeline no longer used
      // === PHASE 6: Pipeline Debug Keys ===

      // Shift+F6: Pass isolation (cycle: off -> pass 0 -> pass 1 -> ... -> pass N -> off)
      if (wParam == VK_F6 && (GetKeyState(VK_SHIFT) & 0x8000)) {
        if (openGl_.pipeline.loaded) {
          int maxPass = static_cast<int>(openGl_.pipeline.compiledPasses.size()) - 1;
          openGl_.pipeline.pipelineDebugIsolatePass++;
          if (openGl_.pipeline.pipelineDebugIsolatePass > maxPass) openGl_.pipeline.pipelineDebugIsolatePass = -1;
          std::wstring msg;
          if (openGl_.pipeline.pipelineDebugIsolatePass < 0) {
            msg = L"Pass Isolation: OFF (full pipeline)";
          } else {
            msg = L"Pass Isolation: render up to Pass " + std::to_wstring(openGl_.pipeline.pipelineDebugIsolatePass);
          }
          SetToast(msg);
          PushLog(L"OPENGL", L"DEBUG", msg, ColorRole::UserInput, L"opengl", L"debug", L"mode=isolate pass=" + std::to_wstring(openGl_.pipeline.pipelineDebugIsolatePass));
        }
        InvalidateRect(hwnd, nullptr, FALSE);
        return 0;
      }

      // Shift+F7: Generate full pipeline debug report
      if (wParam == VK_F7 && (GetKeyState(VK_SHIFT) & 0x8000)) {
        if (openGl_.pipeline.loaded) {
          openGl_.pipeline.pipelineDebugGenerateReport = true;
          SetToast(L"Generating pipeline debug report...");
          PushLog(L"OPENGL", L"DEBUG", L"Pipeline debug report requested.", ColorRole::UserInput, L"opengl", L"debug", L"action=report");
        }
        InvalidateRect(hwnd, nullptr, FALSE);
        return 0;
      }

      // Shift+F8: Toggle pixel validation (16x16 readback after each pass)
      if (wParam == VK_F8 && (GetKeyState(VK_SHIFT) & 0x8000)) {
        if (openGl_.pipeline.pipelineDebugPixelValidate == 6) {
          openGl_.pipeline.pipelineDebugPixelValidate = 0;
        } else {
          openGl_.pipeline.pipelineDebugPixelValidate = 6;
        }
        std::wstring msg = (openGl_.pipeline.pipelineDebugPixelValidate == 6)
          ? L"Pixel Validation: ON (16x16 readback per pass)"
          : L"Pixel Validation: OFF";
        SetToast(msg);
        PushLog(L"OPENGL", L"DEBUG", msg, ColorRole::UserInput, L"opengl", L"debug", L"mode=pixel_validate");
        InvalidateRect(hwnd, nullptr, FALSE);
        return 0;
      }

      // Shift+F9: Generate full Phase 8 runtime validation report
      if (wParam == VK_F9 && (GetKeyState(VK_SHIFT) & 0x8000)) {
        if (openGl_.pipeline.loaded) {
          openGl_.pipeline.pipelineDebugGenerateReport = true;
          SetToast(L"Generating Phase 8 runtime validation report...");
          PushLog(L"OPENGL", L"DEBUG", L"Phase 8 runtime validation report requested.", ColorRole::UserInput, L"opengl", L"debug", L"action=phase8_report");
        }
        InvalidateRect(hwnd, nullptr, FALSE);
        return 0;
      }

      // Shift+F10: Dynamic test mode (resolution change + preset reload cycles)
      if (wParam == VK_F10 && (GetKeyState(VK_SHIFT) & 0x8000)) {
        if (openGl_.pipeline.loaded) {
          openGl_.pipeline.dynamicTestActive = !openGl_.pipeline.dynamicTestActive;
          openGl_.pipeline.dynamicTestStep = 0;
          std::wstring msg = openGl_.pipeline.dynamicTestActive
            ? L"Dynamic Test: ON (resolution + preset cycling)"
            : L"Dynamic Test: OFF";
          SetToast(msg);
          PushLog(L"OPENGL", L"DEBUG", msg, ColorRole::UserInput, L"opengl", L"debug", L"mode=dynamic_test");
        }
        InvalidateRect(hwnd, nullptr, FALSE);
        return 0;
      }
...
RECT MonixApp::ContentRect(const RECT& clientRect) const {
  return RECT { 72, 118, clientRect.right - 72, clientRect.bottom - 66 };
}
...
void MonixApp::DrawFooter(HDC dc, const RECT& clientRect) {
  ...
  // Dead GL code removed - shader pipeline no longer used
  {
    shaderName = paths_.crtShaderFile.stem().wstring();
  }
  ...
  const std::wstring footerText =
    ...
    L"F1 status | F2 pause | F3 view | F4 shader | F5 reload | F6/F7 export | F8 triage | F9 font | F10 demo";
...
void MonixApp::DrawIntro(HDC dc, const RECT& clientRect) {
  ...
  DrawTextRect(dc, RECT { clientRect.left, clientRect.bottom - 140, clientRect.right, clientRect.bottom - 88 }, L"Booting native telemetry, CRT overlay, S.C.R.A.M watcher and kernel probes...", ColorRole::Dim, bodyFont_, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
}
...
void MonixApp::DrawNetworkView(HDC dc, const RECT& clientRect) {
  ...
  DrawTextRect(dc, RECT { header.left + 360, header.top, header.right - 120, header.bottom }, L"REMOTE", ColorRole::Primary, smallFont_, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
  DrawTextRect(dc, RECT { header.right - 110, header.top, header.right - 12, header.bottom }, L"STATE", ColorRole::Primary, smallFont_, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
...
void MonixApp::DrawSettingsView(HDC dc, const RECT& clientRect) {
  ...
  if (showGeneral) {
    ...
    DrawPanel(dc, leftPanel, L"CRT EFFECTS", ColorRole::Primary, smallFont_);
    DrawPanel(dc, rightTop, L"ADVANCED CRT", ColorRole::Thermal, smallFont_);
    DrawPanel(dc, rightMid, L"COLOR CORRECTION", ColorRole::Warning, smallFont_);
    DrawPanel(dc, rightMid2, L"SCANLINE STYLE", ColorRole::Network, smallFont_);
    DrawPanel(dc, rightBottom, L"BURN-IN", ColorRole::Scram, smallFont_);
    ...
    drawRows(FullCrtSettings(), ColorRole::Primary, ColorRole::White);
    drawRows(AdvancedCrtSettings(), ColorRole::Thermal, ColorRole::Thermal);
    drawRows(ColorCorrectionSettings(), ColorRole::Warning, ColorRole::Warning);
    drawRows(ScanlineStyleSettings(), ColorRole::Network, ColorRole::Network);
    drawRows(BurnInSettings(), ColorRole::Scram, ColorRole::Scram);
    ...
    drawFilterTab(L"SLANGP", ShaderLanguageFilter::SlangPreset);
    ...
    if (shaderBrowserPanel_ && shaderBrowserPanel_->hasSelection()) {
      ...
      if (!detail.firstError.empty()) {
        RECT errPanel { detailPanel.left, detailPanel.bottom - 170, detailPanel.right, detailPanel.bottom - 58 };
