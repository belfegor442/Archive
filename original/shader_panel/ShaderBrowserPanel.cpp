...
ShaderLanguage ShaderBrowserPanel::languageFromFilter() const {
    ...
    case ShaderLanguageFilter::SlangPreset:return ShaderLanguage::Slang;
...
bool ShaderBrowserPanel::isLanguageVisible(ShaderLanguage lang) const {
    ...
    if (state_.filter == ShaderLanguageFilter::SlangPreset) {
        return false;
    }
...
std::vector<size_t> ShaderBrowserPanel::filteredIndices() const {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (state_.filter == ShaderLanguageFilter::SlangPreset) {
            if (e->extension == ".slangp" && matchesSearch(*e)) {
                result.push_back(i);
            }
            continue;
        }
        ...
        if (e->language == target && e->extension != ".slangp" && matchesSearch(*e)) {
            result.push_back(i);
        }
    }
